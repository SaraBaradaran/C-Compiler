...
globalvar:
		 ...
		 GLOBAL INT ID ASSIGN exp SEMICOLON 
		{
			...
			datafile = fopen("mipsfile.txt", "a+");
			
			if(ISNUMBER(srcreg))
				fprintf(datafile, "\taddi %s, $zero, %s \n", x->reg, srcreg);
			else{
				fprintf(datafile, "\tmove %s, %s \n", x->reg, srcreg);
				if(srcreg[1] == 't')
...
function: 
		...
		} CLOSEP OPENA stmt
		{
			...
			if(strcmp($2,"main") != 0){
				for(int i=0; i<=7; i++)
					fprintf(datafile, "\tlw $s%d, %d($sp)\n", i, i*4);
				fprintf(datafile, "\taddi $sp, $sp , 32\n");	
				fprintf(datafile, "\tjr $ra\n");
			}
			else
				fprintf(datafile, "\tli $v0, 10\n\tsyscall\n");	
			fclose(datafile);
		}
...
print:	
		PRINT OPENP exp CLOSEP SEMICOLON
		{
			...
			if(strcmp(currentScope, "main") != 0){
				fprintf(datafile, "\taddi $sp, $sp , -4\n");
				fprintf(datafile, "\tsw $a0, 0($sp)\n");
				
			}
			
			if(ISNUMBER(srcreg))
				fprintf(datafile, "\taddi $a0, $zero, %s \n", srcreg);
				
			else
				fprintf(datafile, "\tmove $a0, %s \n", srcreg);
				
			fprintf(datafile, "\tli $v0, 1\n\tsyscall\n");
			
			if(strcmp(currentScope, "main") != 0){
			
				fprintf(datafile, "\tlw $a0, 0($sp)\n");
				fprintf(datafile, "\taddi $sp, $sp , 4\n");
		...
		ID OPENB exp CLOSEB ASSIGN exp SEMICOLON
		{
			...
			if(x == NULL){
				...
				if(ISNUMBER(srctreg1) && ISNUMBER(srctreg2)){
					...
					fprintf(datafile, "\tsll %s, %s , 2\n", treg1, srctreg1);
					fprintf(datafile, "\tadd %s, %s , %s\n", treg1, treg1, x->reg);
					fprintf(datafile, "\taddi %s, $zero , %s\n", treg2, srctreg2);
					fprintf(datafile, "\tsw %s, 0(%s)\n", treg2, treg1);
				}
				else{
...
array_declaration:
		...
		INT ID OPENB exp CLOSEB SEMICOLON
		{
			...
			if(ISNUMBER(srcreg)){
				fprintf(datafile, "\taddi $a0, $zero, %d \n", atoi(srcreg)*4);	
			}
				
			else{
				fprintf(datafile, "\tsll $a0, %s , 2\n", srcreg);
				if(srcreg[1] == 't')
					SetFreeRegister(srcreg);

			}
			fprintf(datafile, "\tli $v0, 9 \n\tsyscall\n");
			fprintf(datafile, "\tmove %s, $v0\n", x->reg);
			...
			if(x == NULL){
				...
				if(x->arg_num != $3){
					...
					for(int i=0; i<$3; i++){
						...
						if(ISNUMBER(arg[i]))
							fprintf(datafile, "\taddi $a%d, $zero , %s\n",i, arg[i]);		
						else					
							fprintf(datafile, "\tmove $a%d, %s\n",i, arg[i]);
					}
					
					for(int i=0; i<$3; i++)
						free(arg[i]);
					free(arg);	
					fprintf(datafile, "\tjal %s\n", $1);
...
loop:								
		...
		} OPENP exp 
		{
			...
			datafile = fopen("mipsfile.txt", "a+");
			label_num++;
			fprintf(datafile, "\tbeq %s, $zero, L%d\n", destreg, label_num);
			fclose(datafile);
		...
		{
			datafile = fopen("mipsfile.txt", "a+");
			fprintf(datafile, "\tj LOOP%d\n", loop_num);
			fclose(datafile);

			datafile = fopen("mipsfile.txt", "a+");
			fprintf(datafile, "L%d: \n", label_num);
			fclose(datafile);
...
exp :	  	exp BLT exp		
		...
		{ 
			...
			if(r1 && r2)
				...
				fprintf(datafile, "\tsub %s, $zero , %s", treg, srctreg2);
				fprintf(datafile, "\taddi %s, %s , %s \n", treg, treg, srctreg1);
			}
			else if(r2)
				fprintf(datafile, "\taddi %s, %s , -%s \n", treg, srctreg1, srctreg2);
			else
				fprintf(datafile, "\tsub %s, %s , %s \n", treg, srctreg1, srctreg2);
		...
		{ 	
			...
			if(r1 && r2)
				...
				fprintf(datafile, "\tslti %s, $zero , %s \n", treg1, srctreg1);
				fprintf(datafile, "\taddi %s, $zero , %s\n", treg2, srctreg1);
				fprintf(datafile, "\tslt %s, %s , $zero \n", treg2, treg2);
				fprintf(datafile, "\tor %s, %s , %s \n", treg1, treg1, treg2);
				label_num++;
				fprintf(datafile, "\tbeq %s, $zero , L%d \n", treg1, label_num);
				fprintf(datafile, "\tslt %s, $zero , %s \n", treg1, srctreg2);
				fprintf(datafile, "\tslt %s, %s , $zero \n", treg2, srctreg2);
				fprintf(datafile, "\tor %s, %s , %s \n", treg1, treg1, treg2);
				fprintf(datafile, "\tL%d:\n", label_num);
			}
		...
		{ 
			...
			if(r1 && r2)
				...
				fprintf(datafile, "\tor %s, %s , %s \n", treg1, treg1, treg2);
				label_num++;
				fprintf(datafile, "\taddi %s, $zero , 1 \n", treg2);
				fprintf(datafile, "\tbeq %s, %s , L%d \n", treg2, treg1, label_num);
				fprintf(datafile, "\tslt %s, $zero , %s \n", treg1, srctreg2);
				fprintf(datafile, "\tslt %s, %s , $zero \n", treg2, srctreg2);
				fprintf(datafile, "\tor %s, %s , %s \n", treg1, treg1, treg2);
				fprintf(datafile, "\tL%d:\n", label_num);
		...
		{ 
			...
			_Bool r2 = ISNUMBER(srctreg2);
			if(r1 && r2)
				fprintf(datafile, "\tori %s, $zero , %d \n", treg, atoi(srctreg1) | atoi(srctreg2));
			else if(r1)
				fprintf(datafile, "\tori %s, %s , %s \n", treg, srctreg2, srctreg1);
			else if(r2)
				fprintf(datafile, "\tori %s, %s , %s \n", treg, srctreg1, srctreg2);
			else		
				fprintf(datafile, "\tor %s, %s , %s \n", treg, srctreg1, srctreg2);	
		...
		{ 
			...
			_Bool r2 = ISNUMBER(srctreg2);
			if(r1 && r2)
				fprintf(datafile, "\tandi %s, $zero , %d \n", treg, atoi(srctreg1) & atoi(srctreg2));
			else if(r1)
				fprintf(datafile, "\tandi %s, %s , %s \n", treg, srctreg2, srctreg1);
			else if(r2)
				fprintf(datafile, "\tandi %s, %s , %s \n", treg, srctreg1, srctreg2);
			else		
				fprintf(datafile, "\tand %s, %s , %s \n", treg, srctreg1, srctreg2);	
		...
		| ID OPENB exp CLOSEB
		{
			...
			if(x == NULL){
				...
				if(ISNUMBER(srcreg)){
					...
					fprintf(datafile, "\tlw %s, %d(%s)\n", treg, atoi(srcreg)*4, x->reg);
				}
				else{
				
					fprintf(datafile, "\tsll %s, %s , 2\n", treg, srcreg);
					fprintf(datafile, "\tadd %s, %s , %s\n", treg, treg, x->reg);
					fprintf(datafile, "\tlw %s, 0(%s)\n", treg, treg);
				}
				pushStack(treg);
				fclose(datafile);
			}
...
int main(int argc, char** argv){
	...
	FILE* datafile = fopen("mipsfile.txt", "w");
	if(datafile == 0){
